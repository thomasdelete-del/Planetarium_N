...
function _gaiaGrenzmag(z){
  ...
  if(basis>=6.49)return katalogMax;
...
function _gaiaStreamPruefen(zellen,bedarf){
  ...
  const gRa=_gaiaStream.groupRa||4,gDe=_gaiaStream.groupDec||4,wanted=new Set();
  ...
  for(const key of wanted){
    ...
    if(_gaiaKachelnLaden.has(key)||_gaiaKachelnLaden.size>=4)continue;
    const tile=level.byKey&&level.byKey.get(key);if(!tile)continue;
    _gaiaKachelnLaden.add(key);
    fetch(tile.file+"?v="+_gaiaManifestVersion).then(r=>r.ok?r.arrayBuffer():Promise.reject(new Error("HTTP "+r.status))).then(ab=>{
      if(_gaiaStreamLevel===level.magnitude){
        _gaiaKacheln.set(key,{catalog:gaiaVektorKachelLesen(ab),used:++_gaiaStreamTick});
        if(_gaiaKacheln.size>GAIA_STREAM_CACHE_MAX){
...
function _gaiaStart(){
  ...
  fetch("gaia/manifest.json?v=5").then(r=>r.ok?r.json():Promise.reject()).then(m=>{
    if(!m||m.strategy!=="cumulative"||!Array.isArray(m.stages)||!m.stages.length)throw new Error("kein Stufenmanifest");
    _gaiaManifestVersion=m.version||1;_gaiaStufen=m.stages;_gaiaStreamSetzen(m.streaming);_gaiaStufeLaden(0);
  }).catch(()=>_gaiaAltStart());
}
...
if(op!==curOp){g.globalAlpha=op;curOp=op}if(rad<1.1||rad<=rbg*.96){g.fillRect(x-rad,y-rad,rad*2,rad*2)}else{g.beginPath();g.arc(x,y,rad,0,Math.PI*2);g.fill()}}};const _gM=_GAIA?_vondrak(jd0):null;
  ...
  const _gaiaFast=typeof interacting!=="undefined"&&interacting>0;
  if(_gaiaFast&&(window.skyMagBase||6.5)<6.49)bgLimit=Math.min(bgLimit,8.5);
  ...
  const _gaiaLODStride=Math.max(1,Math.round(36/Math.max(1,zEff*zEff)));
  const drawGaia=(zk,katalog)=>{
    ...
    const dichteBoost=Math.max(1,Math.min(5,Math.sqrt((b-a)/180)));
...
(function initFastSkyInteraction(){
  ...
  cv.addEventListener("pointerdown",e=>{active.add(e.pointerId);interacting=8;__requestPlanetariumFrame()},{passive:true});
