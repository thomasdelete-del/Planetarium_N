...
void hide(){gl_Position=vec4(2.0,2.0,0.0,1.0);gl_PointSize=0.0;v_alpha=0.0;}
void main(){
  float mag=a_mag;if(mag>u_lod.x){hide();return;}
  ...
  if(mag>6.5&&stride>1.0&&mod(a_id,stride)>=1.0){hide();return;}
  ...
  gl_PointSize=max(1.0,2.0*radius*u_point);v_col=a_col;
}`);
    const fs=_gaiaGLShader(gl,gl.FRAGMENT_SHADER,`precision mediump float;varying vec3 v_col;varying float v_alpha;
void main(){vec2 q=gl_PointCoord-.5;float d=dot(q,q);if(d>.25)discard;float edge=smoothstep(.25,.12,d);gl_FragColor=vec4(v_col,v_alpha*edge);}`);
...
function _gaiaGLUpload(G){
  ...
  try{const gl=_gaiaGL.gl,stride=9,data=new Float32Array(G.N*stride);let zk=0;
    ...
    const buffer=gl.createBuffer();gl.bindBuffer(gl.ARRAY_BUFFER,buffer);gl.bufferData(gl.ARRAY_BUFFER,data,gl.STATIC_DRAW);const eintrag={buffer,count:G.N};_gaiaGL.buffers.set(G,eintrag);return eintrag;
...
function _gaiaGLDraw(kataloge,o){
  const eintraege=[];for(const G of kataloge||[]){const e=_gaiaGLUpload(G);if(e)eintraege.push(e)}if(!eintraege.length)return false;
  try{const gl=_gaiaGL.gl,c=_gaiaGL.canvas;c.style.display="block";if(c.width!==cvW||c.height!==cvH){c.width=cvW;c.height=cvH}gl.viewport(0,0,c.width,c.height);gl.clearColor(0,0,0,0);gl.clear(gl.COLOR_BUFFER_BIT);gl.useProgram(_gaiaGL.program);
    ...
    gl.enable(gl.BLEND);gl.blendFunc(gl.SRC_ALPHA,gl.ONE_MINUS_SRC_ALPHA);let gesamt=0;for(const e of eintraege){gl.bindBuffer(gl.ARRAY_BUFFER,e.buffer);attr("a_pos",3,0);attr("a_mag",1,3);attr("a_col",3,4);attr("a_id",1,7);attr("a_density",1,8);gl.drawArrays(gl.POINTS,0,e.count);gesamt+=e.count}gl.flush();
...
let __loopPending=false,__lastRenderedJD=null,__lastLabelTS=0,__lastAstronomyTS=0;
...
function __astronomyPixelMinutes(){
  ...
  /* Bei starkem Zeitraffer sind bis zu zwei Pixel Abstand zwischen exakten
     Gaia-Projektionen unauffaellig, sparen aber einen grossen Teil der Vollbilder. */
  const targetPx=Math.abs(speed)>=3600?2:Math.abs(speed)>=900?1.35:1;
  return Math.max(.01,Math.min(4,targetPx/Math.max(.001,pxRad*Math.PI/720)));
}
function __astronomyFrameInterval(){const s=Math.abs(speed);return s>=3600?33:s>=900?24:0}
...
(function initFastSkyInteraction(){
  ...
  const end=e=>{if(!active.delete(e.pointerId))return;window.__gaiaVerticalPan=false;/* Der Abschlussframe muss als aktive Aenderung gelten; mit 0 verwarf der Scheduler ihn. */interacting=2;__requestPlanetariumFrame()};
