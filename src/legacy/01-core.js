...
function __astronomyFrameInterval(){const s=Math.abs(speed),p=window.__devicePerformanceProfile&&window.__devicePerformanceProfile.level;if(s>=3600){/* Am Tag fehlen die teuren Gaia-/Milchstrassenebenen weitgehend. Die hohe
...
function ensureEclBox(){
  ...
  b.querySelector(".ei-next").onclick=function(e){e.preventDefault();e.stopPropagation();
    try{jumpToEclipse(1,"solar")}catch(err){}};
  b.querySelector(".ei-zu").onclick=function(e){e.preventDefault();e.stopPropagation();window.__eclZu=true;window.__eclipseNavigation=null;verbergeEclBox()};
  return b;
}
...
function zeigeEclBox(datum,stadt,land,proz){
  ...
  window.__eclipseNavigation={stadt:stadt,land:land||""};
  b.querySelector(".ei-date").textContent=datum;
  b.querySelector(".ei-place").textContent=stadt+(land?", "+land:"");
  b.querySelector(".ei-cover").textContent=proz+"\u202F% bedeckt";
...
function eclBoxAuto(){
  ...
  try{
    ...
    if(window.__eclipseNavigation){
      ...
      zeigeEclBox(ds,nav.stadt,nav.land,Math.round(Math.max(0,Math.min(100,r.mag*100))));
      return;
    }
    if(r.mag>.02&&r.h>-1){
...
const date=new Date((jd-2440587.5)*864e5);simYear=date.getUTCFullYear();utcBase=tzFromLng(lng);const approxLocal=new Date(date.getTime()+utcBase*3600*1e3);const approxDoy=date2doy(approxLocal.getUTCDate(),approxLocal.getUTCMonth(),approxLocal.getUTCFullYear());const inEU=lng>=-10&&lng<=40&&lat>=34&&lat<=72;applyDST(inEU&&euDSTactive(simYear,approxDoy));const localMs=date.getTime()+utcOff*3600*1e3;const ld=new Date(localMs);simDay=date2doy(ld.getUTCDate(),ld.getUTCMonth(),simYear);simMin=ld.getUTCHours()*60+ld.getUTCMinutes()+ld.getUTCSeconds()/60;syncYearUI();document.getElementById("sTime").value=Math.round(simMin);document.getElementById("dayslider").value=simDay;updLabels();paused=false;lastT=null;speed=type==="solar"?1:120;const spSlider=document.getElementById("sSpd");if(spSlider){const pos=Math.log(speed)/Math.log(3600)*1e3;spSlider.value=Math.round(pos)}if(typeof setSp==="function")setSp(spSlider?spSlider.value:585);if(type==="solar"&&typeof pointObserverAtSun==="function")pointObserverAtSun();const pb=document.getElementById("ba");if(pb){pb.textContent="⏸ Pause";pb.classList.add("on")}const tn=type==="solar"?"Sonnenfinsternis":"Mondfinsternis";const ds=`${ld.getUTCDate()}.${ld.getUTCMonth()+1}.${simYear}`;
const ort=finOrt?` · ${finOrt.stadt.n}${finOrt.stadt.land?`, `+finOrt.stadt.land:``} · ${Math.round(Math.min(100,finOrt.mag*100))} % bedeckt`:(type==="solar"?" · von keiner Stadt der Liste aus sichtbar":"");showToast(`${dir>0?"Nächste":"Vorherige"} ${tn}: ${ds}${ort} — ${type==="solar"?"Beginn der Verdeckung · Echtzeit":"läuft"}`);
  if(type==="solar"&&finOrt)zeigeEclBox(ds,finOrt.stadt.n,finOrt.stadt.land,0);
