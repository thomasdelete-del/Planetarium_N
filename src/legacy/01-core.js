...
function _gaiaDichteLaden(meta){
  ...
  fetch(meta.file+"?v="+_gaiaManifestVersion).then(r=>r.ok?r.arrayBuffer():Promise.reject()).then(buf=>{
    ...
    _gaiaDichte={n,v,light,max,stride,gRA,gDE,verz,boost,sample:true};if(typeof draw==="function"&&W)draw();
  }).catch(()=>{});
}
...
if(_gaiaDichte&&_gaiaDichte.sample&&(window.skyMagBase||6.5)>=6.49&&nightF>.18&&!(typeof interacting!=="undefined"&&interacting>0)){
...
if(op!==curOp){g.globalAlpha=op;curOp=op}if(rad<1.1||rad<=rbg*.96){g.fillRect(x-rad,y-rad,rad*2,rad*2)}else{g.beginPath();g.arc(x,y,rad,0,Math.PI*2);g.fill()}}};const _gM=_GAIA?_vondrak(jd0):null;
  ...
  const _gaiaFast=(typeof interacting!=="undefined"&&interacting>0)||window.__skyRenderQuality<2;
  ...
  const _gaiaLODStride=Math.max(window.__skyRenderQuality===0?3:window.__skyRenderQuality===1?2:1,Math.round(16/Math.max(1,zEff*zEff)));
  const drawGaia=(zk,katalog)=>{
    ...
    for(let i=a;i<b;i++){
      ...
      const baseOp=mag<2?.98:mag<3.5?.88:mag<5?.68:mag<6.5?.48:
        Math.max(.035,.30*Math.pow(10,-.18*faint))*lodFlux*telescopeGain;
      const rad=mag<2?rbg*1.55:mag<3.5?rbg*1.30:mag<5?rbg*1.08:
...
queueMicrotask(()=>{
  ...
  if(g&&!g.__nameLayersInstalled){
    ...
    g.fillText=function(text,x,y,maxWidth){
      ...
      if(cameraOnly&&objectLabels.has(label)&&!brightestCameraStars.has(label)&&!cameraBodies.has(label))return;
      if(constellationLabels.has(label)&&(!showConstellationNames||(typeof orientMode!=="undefined"&&orientMode)||cameraOnly))return;
      if(objectLabels.has(label)&&!showObjectNames)return;
