...
const CITY_TZ_BY_COUNTRY={"Deutschland":"Europe/Berlin","Österreich":"Europe/Vienna","Schweiz":"Europe/Zurich","Großbritannien":"Europe/London","Frankreich":"Europe/Paris","Spanien":"Europe/Madrid","Italien":"Europe/Rome","Niederlande":"Europe/Amsterdam","Belgien":"Europe/Brussels","Portugal":"Europe/Lisbon","Griechenland":"Europe/Athens","Schweden":"Europe/Stockholm","Norwegen":"Europe/Oslo","Dänemark":"Europe/Copenhagen","Finnland":"Europe/Helsinki","Island":"Atlantic/Reykjavik","Irland":"Europe/Dublin","Polen":"Europe/Warsaw","Tschechien":"Europe/Prague","Ungarn":"Europe/Budapest","Türkei":"Europe/Istanbul","Mexiko":"America/Mexico_City","Brasilien":"America/Sao_Paulo","Argentinien":"America/Argentina/Buenos_Aires","Peru":"America/Lima","Chile":"America/Santiago","Ägypten":"Africa/Cairo","Südafrika":"Africa/Johannesburg","Kenia":"Africa/Nairobi","Nigeria":"Africa/Lagos","Marokko":"Africa/Casablanca","Japan":"Asia/Tokyo","China":"Asia/Shanghai","Indien":"Asia/Kolkata","Thailand":"Asia/Bangkok","Singapur":"Asia/Singapore","Südkorea":"Asia/Seoul","VAE":"Asia/Dubai","Israel":"Asia/Jerusalem","Neuseeland":"Pacific/Auckland","Ukraine":"Europe/Kyiv","Belarus":"Europe/Minsk","Rumänien":"Europe/Bucharest","Bulgarien":"Europe/Sofia","Slowakei":"Europe/Bratislava","Slowenien":"Europe/Ljubljana","Kroatien":"Europe/Zagreb","Serbien":"Europe/Belgrade","Lettland":"Europe/Riga","Litauen":"Europe/Vilnius","Estland":"Europe/Tallinn"};
const CITY_TZ_SPECIAL={"New York":"America/New_York","Washington":"America/New_York","Chicago":"America/Chicago","Los Angeles":"America/Los_Angeles","Ottawa":"America/Toronto","Toronto":"America/Toronto","Moskau":"Europe/Moscow","Sankt Petersburg":"Europe/Moscow","Hongkong":"Asia/Hong_Kong","Sydney":"Australia/Sydney","Canberra":"Australia/Sydney","Melbourne":"Australia/Melbourne"};
let activeTimeZone=null;
function cityTimeZone(c){return c&&(CITY_TZ_SPECIAL[c.n]||CITY_TZ_BY_COUNTRY[c.land])||null}
function zoneOffsetAtInstant(tz,date){try{const parts=new Intl.DateTimeFormat("en-CA",{timeZone:tz,year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",second:"2-digit",hourCycle:"h23"}).formatToParts(date),v={};parts.forEach(p=>{if(p.type!=="literal")v[p.type]=+p.value});return(Date.UTC(v.year,v.month-1,v.day,v.hour,v.minute,v.second)-date.getTime())/36e5}catch(e){return null}}
function zoneOffsetsForLocal(tz,year,doy,minutes){if(!tz||year<1900||year>9999)return null;const md=doy2date(doy,year),localMs=Date.UTC(year,md.m,md.d,Math.floor(minutes/60),Math.floor(minutes%60));let actual=zoneOffsetAtInstant(tz,new Date(localMs));if(actual===null)return null;actual=zoneOffsetAtInstant(tz,new Date(localMs-actual*36e5));const jan=zoneOffsetAtInstant(tz,new Date(Date.UTC(year,0,15,12))),jul=zoneOffsetAtInstant(tz,new Date(Date.UTC(year,6,15,12))),base=Math.min(jan==null?actual:jan,jul==null?actual:jul);return{base:base,dst:actual-base,total:actual}}
function formatUtcOffset(v){const sign=v<0?"−":"+",a=Math.abs(v),h=Math.floor(a),m=Math.round((a-h)*60);return"UTC"+sign+h+(m?":"+String(m).padStart(2,"0"):"")}
let utcBase=tzFromLng(lng),dstOffset=0,utcOff=utcBase;window.telescope=false;let paused=false,speed=136,userSpeed=136,showNames=true,showHorizon=false,showAlt=false,showISS=false,zoom=1,showRA=false,showLines=true,showRefCircles=true,showTwilight=false,showZodiac=true;let showMeteors=true,showJMoons=true;let viewMode="dome",camAz=0,camAlt=26,camFov=65,showGround=true;let focusConstellation=null;let meteorParticles=[],lastMeteorT=0,meteorSpawnAcc=0;let BSC=[];let bscPrecYear=null;let lastBscPrec=0;let bscPrecTargetYear=null;let bscPrecCursor=0;let gaiaPrecTargetYear=null;let gaiaPrecCursor=0;window.skyMagBase=(()=>{try{const m=parseFloat(localStorage.getItem("planetarium-sky-mag"));if(m===4.5||m===5.5||m===6.5)return m}catch(e){}return 5.5})();
...
function setPareyProfile(){
  activeTimeZone=null;lat=52.678;lng=12.247;selCity=null;
...
/* Physikalischer Phasenwinkel am Mond. Die bisherige Formel aus der reinen
   ekliptikalen Laengendifferenz ignorierte Mondbreite und Entfernungen. Hier
   wird das Dreieck Sonne-Erde-Mond benutzt; der beleuchtete Scheibenanteil ist
   danach (1+cos(Phasenwinkel))/2. */
function moonPhaseGeometry(jd0){
  const m=moonEcl(jd0),dl=(m.lon-sunLon(jd0))*Math.PI/180,b=m.lat*Math.PI/180;
  const elong=Math.acos(Math.max(-1,Math.min(1,Math.cos(b)*Math.cos(dl))));
  const T=(jd0-2451545)/36525,M=(357.52911+35999.0502909*T)*Math.PI/180;
  const earthSunKm=149597870.7*(1.000001018*(1-.01671123*Math.cos(M)));
  const earthMoonKm=m.dist;
  const sunMoonKm=Math.sqrt(earthSunKm*earthSunKm+earthMoonKm*earthMoonKm-2*earthSunKm*earthMoonKm*Math.cos(elong));
  const cosPhase=Math.max(-1,Math.min(1,(sunMoonKm*sunMoonKm+earthMoonKm*earthMoonKm-earthSunKm*earthSunKm)/(2*sunMoonKm*earthMoonKm)));
  const phaseAngle=Math.acos(cosPhase),phaseDeg=phaseAngle*180/Math.PI;
  /* Normierter Lichtstrom einer empirischen Mond-Phasenkurve. Er bleibt
     getrennt vom beleuchteten Flaechenanteil fuer eine spaetere Tonwertkurve. */
  const flux=Math.pow(10,-.4*(.026*phaseDeg+4e-9*Math.pow(phaseDeg,4)));
  return{illum:(1+cosPhase)/2,phaseAngle:phaseAngle,flux:flux,elong:elong};
}
moonIllum=function(jd0){return moonPhaseGeometry(jd0).illum};
window.moonPhaseGeometry=moonPhaseGeometry;
...
function moonBrightLimbAngle(mtopo,sunRD,HR,mP,jd0){
  if(_moonLimbCache.jd===null||Math.abs(jd0-_moonLimbCache.jd)>=1/1440){
...
/* Die Beleuchtungsrichtung und die Orientierung der Mondachse sind getrennte
   Größen. Der IAU-Mondpol wird in die aktuelle Beobachterprojektion übertragen;
   die kleine Rechnung wird höchstens einmal pro simulierter Stunde erneuert. */
let _moonAxisCache={jd:null,ang:0};
function moonAxisScreenAngle(mtopo,HR,mP,jd0){
  if(_moonAxisCache.jd!==null&&Math.abs(jd0-_moonAxisCache.jd)<1/24)return _moonAxisCache.ang;
  const T=(jd0-2451545)/36525,d2r=Math.PI/180,ar=mtopo.ra*15*d2r,dr=mtopo.dec*d2r;
  const apr=(269.9949+.0031*T)*d2r,dpr=(66.5392+.013*T)*d2r;
  const m=[Math.cos(dr)*Math.cos(ar),Math.cos(dr)*Math.sin(ar),Math.sin(dr)];
  const p=[Math.cos(dpr)*Math.cos(apr),Math.cos(dpr)*Math.sin(apr),Math.sin(dpr)];
  const dot=m[0]*p[0]+m[1]*p[1]+m[2]*p[2],t=[p[0]-dot*m[0],p[1]-dot*m[1],p[2]-dot*m[2]];
  const tl=Math.hypot(t[0],t[1],t[2])||1,e=.002,q=[m[0]+e*t[0]/tl,m[1]+e*t[1]/tl,m[2]+e*t[2]/tl],ql=Math.hypot(q[0],q[1],q[2])||1;
  const pra=(Math.atan2(q[1],q[0])*180/Math.PI+360)%360/15,pdec=Math.asin(q[2]/ql)*180/Math.PI,pp=altazXY(pra,pdec,HR);
  _moonAxisCache={jd:jd0,ang:Math.atan2(pp.x-mP.x,-(pp.y-mP.y))};return _moonAxisCache.ang;
}
function drawMoonSurfaceFixed(ctx,image,mx,my,r,brightLimbAng,axisAng){
  ctx.save();ctx.translate(mx,my);ctx.rotate(-brightLimbAng+(axisAng||0));ctx.translate(-mx,-my);
  ctx.drawImage(image,mx-r,my-r,r*2,r*2);ctx.restore();
}
...
/* Einziger Eingang fuer erzwungene Vollbilder. Mehrere Aufrufer innerhalb
   desselben Browserframes werden vom bestehenden Loop zu genau einem draw()
   zusammengefuehrt; "optimal" gewinnt dabei immer vor "normal". */
let __forcedSkyFrame=false,__optimalSkyFrame=false;
function __scheduleSkyDraw(reason,optimal){
  __forcedSkyFrame=true;
  if(optimal)__optimalSkyFrame=true;
  __requestPlanetariumFrame();
}
window.scheduleSkyDraw=(reason,options)=>__scheduleSkyDraw(reason,!!(options&&options.optimal));
let __didacticRefineTimer=null;
function __scheduleDidacticSkyDraw(reason){
  clearTimeout(__didacticRefineTimer);
  /* Das erste Bild kennt bereits alle Zielparameter, reduziert aber fuer genau
     einen Frame nur die teure Gaia-Tiefe. Es ist daher kein alter oder falscher
     Zwischenzustand, sondern sofort die richtige Zielansicht. */
  interacting=Math.max(2,interacting||0);
  window.__skyRenderQuality=0;
  __scheduleSkyDraw(reason||"didactic-fast",false);
  const profile=window.__devicePerformanceProfile&&window.__devicePerformanceProfile.level;
  __didacticRefineTimer=setTimeout(()=>{
    interacting=0;window.__skyRenderQuality=2;
    __scheduleSkyDraw((reason||"didactic")+"-refine",true);
  },profile==="low"?140:70);
}
window.scheduleDidacticSkyDraw=__scheduleDidacticSkyDraw;
/* Dieselben Ephemeriden werden innerhalb eines Vollbilds von Objekten,
   Beschriftungen und Informationsfeldern mehrfach abgefragt. Ein Eintrags-Cache
   vermeidet identische trigonometrische Serien fuer denselben Julianischen Tag. */
const __sunLonRaw=sunLon,__moonTopoRaw=moonTopo,__allPlanetsRaw=allPlanets;
let __sunLonMemoJD=NaN,__sunLonMemo=0,__moonTopoMemoJD=NaN,__moonTopoMemo=null,__planetsMemoJD=NaN,__planetsMemo=null;
sunLon=function(jd0){if(jd0!==__sunLonMemoJD){__sunLonMemoJD=jd0;__sunLonMemo=__sunLonRaw(jd0)}return __sunLonMemo};
moonTopo=function(jd0){if(jd0!==__moonTopoMemoJD){__moonTopoMemoJD=jd0;__moonTopoMemo=__moonTopoRaw(jd0)}return __moonTopoMemo};
allPlanets=function(jd0){if(jd0!==__planetsMemoJD){__planetsMemoJD=jd0;__planetsMemo=__allPlanetsRaw(jd0)}return __planetsMemo};
function __trackMoonObserver(){
  if(!window.__moonPhaseTracking)return;
  const jd0=currentJD(),m=moonTopo(jd0),H=(LST()-m.ra*15)*Math.PI/180;
  const phi=lat*Math.PI/180,dec=m.dec*Math.PI/180;
  const alt=Math.asin(Math.sin(phi)*Math.sin(dec)+Math.cos(phi)*Math.cos(dec)*Math.cos(H));
  const az=Math.atan2(Math.sin(H),Math.cos(H)*Math.sin(phi)-Math.tan(dec)*Math.cos(phi));
  viewMode="real";camAz=((az*180/Math.PI)+360)%360;camAlt=alt*180/Math.PI;
  /* Ruhige Grunddarstellung des Beobachtermodus; neu zentriert wird nur beim
     Wechsel auf den jeweils naechsten Kalendertag. */
  camFov=65;zoom=1;panX=0;panY=0;zoomedObj=null;
}
window.__trackMoonObserver=__trackMoonObserver;
let __moonMeridianTrail=[];
window.__resetMoonMeridianTrail=function(){__moonMeridianTrail=[]};
window.__drawMoonMeridianTrail=function(){
  if(!window.__moonPhaseTracking||!g||!Array.isArray(clickable))return;
  const moon=clickable.find(o=>o&&o.type==="moon");if(!moon)return;
  const key=simYear+"-"+simDay;
  if(!__moonMeridianTrail.length||__moonMeridianTrail[__moonMeridianTrail.length-1].key!==key){
    const geom=moonPhaseGeometry(currentJD()),wax=moonElong(currentJD())<180;
    __moonMeridianTrail.push({key:key,x:moon.sx,y:moon.sy,illum:geom.illum,wax:wax});
    if(__moonMeridianTrail.length>30)__moonMeridianTrail.shift();
  }
  g.save();g.setTransform(1,0,0,1,0,0);
  const n=__moonMeridianTrail.length;
  for(let i=0;i<n-1;i++){
    const p=__moonMeridianTrail[i],age=n-1-i,a=.08+.22*(i/Math.max(1,n-1)),r=6.5*PX;
    g.globalAlpha=a;g.beginPath();g.arc(p.x,p.y,r,0,Math.PI*2);g.fillStyle="#283040";g.fill();
    moonLitPath(g,p.x,p.y,r,p.illum,p.wax);g.fillStyle="#ddd8c9";g.fill();
  }
  g.globalAlpha=.78;g.font=`600 ${11*PX}px Inter,system-ui,sans-serif`;g.textAlign="center";g.textBaseline="top";
  g.fillStyle="#e4d8a8";g.fillText("Komposit · tägliche Meridianpassagen",cv.width/2,10*PX);
  g.restore();
};
let __moonPhaseDayCancel=null;
function stopMoonPhaseDayRun(){
...
function resetStoredSkyImagesOnViewToggle(){
  if(typeof window.__resetMoonMeridianTrail==="function")window.__resetMoonMeridianTrail();
  if(typeof window.__resetSolarYearTrail==="function")window.__resetSolarYearTrail();
}
function startMoonPhaseDayRun(){
  window.__moonPhaseTracking=true;window.__moonPhaseNeedsCenter=false;
  window.__resetMoonMeridianTrail();
  if(typeof orientMode!=="undefined"&&orientMode&&typeof disableOrient==="function")disableOrient();
  viewMode="real";setRealHome();zoom=1;panX=0;panY=0;zoomedObj=null;
  /* Test-Komposit: 100 Grad um 45 Grad Hoehe decken die gesamte sichtbare
     obere Meridianhaelfte vom Horizont bis zum Zenit ab. */
  camAlt=45;camFov=100;
  syncViewModeButtons();if(typeof updateTouchMode==="function")updateTouchMode();
  setSpeedValue(1);setPaused(false);__scheduleDidacticSkyDraw("moon-phase-start");
  if(__moonPhaseDayCancel)return;
  const step=()=>{
    const transit=findMoonTransitNear(currentJD()+1);
    const utcDate=new Date((transit-2440587.5)*864e5);
    simYear=utcDate.getUTCFullYear();utcBase=tzFromLng(lng);
    const approxLocal=new Date(utcDate.getTime()+utcBase*3600e3);
    const approxDoy=date2doy(approxLocal.getUTCDate(),approxLocal.getUTCMonth(),approxLocal.getUTCFullYear());
    const inEU=lng>=-10&&lng<=40&&lat>=34&&lat<=72;
    applyDST(inEU&&euDSTactive(simYear,approxDoy));
    const localDate=new Date(utcDate.getTime()+utcOff*3600e3);
    simYear=localDate.getUTCFullYear();
    simDay=date2doy(localDate.getUTCDate(),localDate.getUTCMonth(),simYear);
    simMin=localDate.getUTCHours()*60+localDate.getUTCMinutes()+localDate.getUTCSeconds()/60;
    const ds=document.getElementById("dayslider");if(ds)ds.value=simDay;
    const ts=document.getElementById("sTime");if(ts)ts.value=Math.round(simMin);
    window.__moonPhaseNeedsCenter=false;updLabels();__scheduleSkyDraw("moon-phase-day",true);
  };
  if(window.__planetariumScheduler)__moonPhaseDayCancel=window.__planetariumScheduler.every("moon-phase-day",step,1000);
  else{const id=setInterval(step,1000);__moonPhaseDayCancel=()=>clearInterval(id)}
}
window.startMoonPhaseDayRun=startMoonPhaseDayRun;
/* Die frueh registrierten Resize-Listener werden auf denselben Planer
   umgebunden. Auch mehrere visualViewport-/orientationchange-Signale fuehren
   damit nicht mehr jeweils unmittelbar eine Vollberechnung aus. */
const __directFitToScreen=fitToScreen;
window.removeEventListener("resize",__directFitToScreen);
if(window.visualViewport)window.visualViewport.removeEventListener("resize",__directFitToScreen);
document.removeEventListener("DOMContentLoaded",__directFitToScreen);
fitToScreen=function(){resize();__scheduleSkyDraw("resize",true)};
window.addEventListener("resize",fitToScreen);
if(window.visualViewport)window.visualViewport.addEventListener("resize",fitToScreen);
document.addEventListener("DOMContentLoaded",fitToScreen,{once:true});
...
function __requestSettledSkyFrame(){
  ...
  __settledSkyFrameTimer=setTimeout(()=>{interacting=0;__scheduleSkyDraw("settled",true)},90);
}
...
function __requestOptimalSkyFrame(){
  clearTimeout(__settledSkyFrameTimer);
  __scheduleSkyDraw("optimal",true);
}
...
function __endGaiaTimeAdjustment(){
  ...
  __settledSkyFrameTimer=setTimeout(()=>{
    interacting=0;window.__gaiaTimeAdjusting=false;__scheduleSkyDraw("time-settled",true);
  },90);
}
...
const __forceThisFrame=__forcedSkyFrame;
if(__optimalSkyFrame){interacting=0;window.__gaiaVerticalPan=false;window.__gaiaTimeAdjusting=false;__optimalSkyFrame=false}
...
if(!paused&&lastT!==null&&!sliderActive&&!(interacting>0)&&_sicht){const dt=Math.min((ts-lastT)/1e3,.08);simMin+=speed*dt/60;if(simMin>=1440){simMin-=1440;simDay++;if(simDay>daysInYear(simYear)){simDay=1;simYear++}}if(simMin<0){simMin+=1440;simDay--;if(simDay<1){simYear--;simDay=daysInYear(simYear)}}if((ts||0)-__lastLabelTS>=(Math.abs(speed)<=2?1000:250)){__lastLabelTS=ts||0;document.getElementById("sTime").value=Math.round(simMin);document.getElementById("dayslider").value=simDay;updLabels()}}if(!paused)lastT=ts;else lastT=null;if(interacting>0)interacting--;{const zr=document.getElementById("telfac");if(zr){const zm=curMag();const t=zoomDisplayText(zm);zr.style.opacity=zm>1.05?1:.55;if(zr.textContent!==t)zr.textContent=t}}{const pb=document.getElementById("bpause");if(pb){const _ez=paused;const t=_ez?"▶":"⏸";const html='<span class="bsym">'+t+'</span>';if(pb.innerHTML!==html)pb.innerHTML=html;pb.classList.toggle("on",!_ez)}}if(orientMode)stepOrient();{const ta="pan-y";if(cv.style.touchAction!==ta)cv.style.touchAction=ta}__wlSync(ts||0);const __jdNow=currentJD(),__timeMoved=__lastRenderedJD===null||Math.abs(__jdNow-__lastRenderedJD)*1440>=__astronomyPixelMinutes(),__frameReady=(ts||0)-__lastAstronomyTS>=__astronomyFrameInterval();const __act=__forceThisFrame||interacting>0||orientMode||sliderActive||(!paused&&__timeMoved&&__frameReady);if(_sicht&&(__act||((ts||0)-__idleTS)>600)){__idleTS=ts||0;__lastAstronomyTS=ts||0;draw();__lastRenderedJD=__jdNow;__forcedSkyFrame=false}
  ...
  if(!paused||interacting>0||orientMode||sliderActive||(__forcedSkyFrame&&_sicht))__requestPlanetariumFrame()
...
function setScene(la,lo,month,day,minute,label,year){
  ...
  activeTimeZone=null;lat=la;lng=lo;simDay=doyFromMonthDay(simYear,month,day);simMin=minute;selCity=null;
...
function setSceneFromJD(la,lo,jd,label){
  ...
  if(typeof window.setYearPlay==="function")window.setYearPlay(false);
  if(typeof setPaused==="function")setPaused(true);
  activeTimeZone=null;lat=la;lng=lo;selCity=null;
...
function jumpScene(id){
  if(id!=="sim-moon-phases"){window.__moonPhaseTracking=false;stopMoonPhaseDayRun()}
  ...
  if(id==="sim-moon-phases"){window.__moonPhaseTracking=true;window.__moonPhaseNeedsCenter=false;jumpMoonPhase(180,"Mondphasenlauf · Vollmond","real");startMoonPhaseDayRun();showToast("Mond täglich im Meridian · feste Beobachter-Grundansicht");return}
...
function curMag(){return orientMode?1:(viewMode==="real"?REAL_HOME_FOV/camFov:zoom)}function zoomDisplayText(zm){const z=zm<10?zm.toFixed(1):String(Math.round(zm));return viewMode==="real"?"🔭 "+z+"× · "+camFov.toFixed(camFov<10?1:0)+"°":"🔭 Ansicht "+z+"×"}
...
function toggleViewMode(){
  ...
  if(viewMode==="real"){
    clearMoonCompositeOnObserverExit();
    viewMode="dome";
  ...
  if(mobile){
    if(api&&typeof api.requestPermission!=="function")enableOrient();
      ...
      if(!api)enableOrientFallback("Kein Lagesensor · manueller Lagemodus");
        ...
        const allow=()=>{document.removeEventListener("pointerdown",allow,true);document.removeEventListener("touchend",allow,true);api.requestPermission().then(s=>{if(s==="granted"){orientMode=false;enableOrient()}else enableOrientFallback("Bewegungssensor nicht erlaubt · manueller Lagemodus")}).catch(()=>enableOrientFallback("Bewegungssensor nicht verfügbar · manueller Lagemodus"))};
        document.addEventListener("pointerdown",allow,true);document.addEventListener("touchend",allow,true);
      }
    }
